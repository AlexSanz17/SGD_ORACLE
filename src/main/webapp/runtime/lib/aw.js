 ...
 if(!window.AW){
	 var AW=function(a,b){return AW.dispatch(a,b)};
	 AW.version=253;
	 AW.toString=function(){return ""}}
	 if(!AW.System){AW.System={}}if(!AW.HTML){AW.HTML={}}
...
(function(){
	...
	AW.log=function(level,arg){
		try{
			var i,s="";
			for(i=0;i<arg.length;i++){s+=arg[i]+" "}
			window.status=s}
	...
	AW.forEach=function(array,handler){var i,custom={};for(i in array){if(!custom[i]){handler(i,array[i])}}};
	AW.element=function(id){if(!id || typeof(id)!="string"){return}var i,e,docs=AW.docs;for(i=0;i<docs.length;i++){e=docs[i].getElementById(id);if(e){return e}}};
	...
	AW.object=function(id,skipContent){
		...
		for(var i=1;i<parts.length;i++)
		{
			...
			if(obj["_"+name+"Content"]){
				if(!skipContent){
					for(var j=i;j<parts.length;j++){obj=obj.getContent(parts[j])}}
					return obj}tag+="-"+name;
					if(AW.element(tag)){
					...
					var index1=parts[++i];
					tag+="-"+index1;
					if(AW.element(tag)){obj=obj.getTemplate(name,index1);continue}
					var index2=parts[++i];tag+="-"+index2;
					if(AW.element(tag)){obj=obj.getTemplate(name,index1,index2);continue}
					var index3=parts[++i];tag+="-"+index3;
					if(AW.element(tag)){obj=obj.getTemplate(name,index1,index2,index3);continue}
		}
		return obj};
...
(function(){
	...
	if(AW.webkit){if(ua.match("Chrome")){AW.chrome=true}
	else if(ua.match("Safari")){AW.safari=true}}
	if(AW.safari && !document.evaluate){AW.safari2=true}
	if(!ua.match("Windows")){AW.unix=true}AW.theme="classic";
	if(ua.match("Windows NT 6")){AW.theme="vista"}
	if(ua.match("Windows NT 5.1")){AW.theme="xp"}
	if(AW.safari || ua.match("Mac OS")){AW.theme="aqua"}AW.strict=(""+document.compatMode).match("CSS");
	if(AW.safari2 ||(AW.safari && !document.compatMode && document.doctype &&(document.doctype.systemId || !(""+document.doctype.publicId).match(/(Transitional|Final)/)))){AW.strict=true}
	if(!AW.strict){AW.quirks=true}var htmlc=" aw-all";
	...
	if(AW.ie){var s=document.documentElement.currentStyle;
	if(s.outlineStyle){AW.ie8=true;htmlc+=" aw-ie8"}
	else if(s.maxWidth){AW.ie7=true;htmlc+=" aw-ie7"}
	else if(s.textOverflow){AW.ie6=true;htmlc+=" aw-ie6"}
	else if(s.writingMode){AW.ie5=true;htmlc+=" aw-ie5"}s=null;
	if(document.documentMode >=8){AW.ms8=true;htmlc+=" aw-ms8"}
	else if(document.documentMode==7 ||(AW.ie7 && AW.strict)){AW.ms7=true;htmlc+=" aw-ms7"}
	else if(AW.ie6 && AW.strict){AW.ms6=true;htmlc+=" aw-ms6"}
	else{AW.ms5=true;htmlc+=" aw-ms5"}}
	if(AW.gecko){
		if(document.elementFromPoint){AW.ff3=true;htmlc+=" aw-ff3"}
		else if(window.globalStorage){AW.ff2=true;htmlc+=" aw-ff2"}
		else if(window.XPCNativeWrapper){AW.ff15=true;htmlc+=" aw-ff15"}
		else{AW.ff1=true;htmlc+=" aw-ff1"}
		if(!AW.ff3){AW.ffx=true;htmlc+=" aw-ffx"}}
	if(AW.ie5 || AW.ie6){htmlc+=" aw-png1 aw-"+AW.theme+"-png1"}
	else{htmlc+=" aw-png2"}AW._htmlClasses=htmlc;
	if(AW.strict){AW.dx=8;AW.dy=4}
	else{AW.dx=0;AW.dy=0}AW.sx=20;AW.sy=20})();

(function(){
	if(AW.ie){
		
		try{document.execCommand("BackgroundImageCache",false,true)}
		catch(err){}AW.attachEvent=function(element,name,handler){return element.attachEvent(name,handler)};
		AW.detachEvent=function(element,name,handler){return element.detachEvent(name,handler)};
		AW.srcElement=function(event){if(event){return event.srcElement}};
		AW.toElement=function(event){if(event){return event.toElement}};
		AW.setReturnValue=function(event,value){if(event){event.returnValue=value}};
		AW.setCapture=function(element){return element.setCapture()};
		AW.releaseCapture=function(element){return element.releaseCapture()};
		AW.addRule=function(stylesheet,selector,rule){return stylesheet.addRule(selector,rule)};
		AW.getRules=function(stylesheet){return stylesheet.rules};AW.setOuterHTML=function(element,html){element.outerHTML=html};
		AW.createXMLHttpRequest=function(){
			try{return new ActiveXObject("MSXML2.XMLHTTP")}
			catch(err){}try{return new ActiveXObject("Microsoft.XMLHTTP")}
			catch(err){}try{return new XMLHttpRequest}
			catch(err){}};
		AW.getLeft=function(element){return element.getBoundingClientRect().left};
		AW.getTop=function(element){return element.getBoundingClientRect().top};
		AW.contains=function(parent,child){return parent.contains(child)}}})();

(function(){
	if(!AW.ie){
		var capture;
		AW.attachEvent=function(target,name,handler){
			if(capture){handler[name]=function(event){
				return handler.call(target,event)};
				window.addEventListener(name.replace(/^on/,""),handler[name],true)}
			else{target.addEventListener(name.replace(/^on/,""),handler,false)}};
		AW.detachEvent=function(target,name,handler){
			if(capture){window.removeEventListener(name.replace(/^on/,""),handler[name],true);
			handler[name]=null}else{target.removeEventListener(name.replace(/^on/,""),handler,false)}};
		AW.srcElement=function(event){try{return(event.target && event.target.nodeType==3)?event.target.parentNode:event.target}catch(e){return event.target}};
		AW.toElement=function(event){try{return(event.relatedTarget && event.relatedTarget.nodeType==3)?event.relatedTarget.parentNode:event.relatedTarget}catch(e){return event.relatedTarget}};
		AW.setReturnValue=function(event,value){if(event && event.preventDefault && !value){event.preventDefault()}};
		AW.setCapture=function(element){capture=element};
		AW.releaseCapture=function(element){capture=null};
		AW.addRule=function(stylesheet,selector,rule){var i=stylesheet.cssRules.length;stylesheet.insertRule(selector+"{"+rule+"}",i);stylesheet.cssRules[i].style.cssText=rule};
		AW.getRules=function(stylesheet){return stylesheet.cssRules};
		AW.setOuterHTML=function(element,html){
			var range=element.ownerDocument.createRange();range.setStartBefore(element);
			var fragment=range.createContextualFragment(html);element.parentNode.replaceChild(fragment,element)};
		AW.createXMLHttpRequest=function(){return new XMLHttpRequest};
		AW.getLeft=function(element){return getRectangle(element).left};
		AW.getTop=function(element){
			...
			var getRectangle=function(e){
				...
				function getPos(el){
					if(!el){
						return{x:0,y:0}}
					if(el==document.body.parentNode){return{x:0,y:0}}
					if(el==document.body){return{x:el.offsetLeft,y:el.offsetTop}}
					var p=el.offsetParent;var pp=getPos(p);
					return{x:el.offsetLeft+pp.x,y:el.offsetTop+pp.y}}
				var pp=getPos(e);return{left:pp.x,right:pp.x+e.offsetWidth,top:pp.y,bottom:pp.y+e.offsetHeight}};
				AW.contains=function(parent,child){while(child){if(parent==child){return true}child=child.parentNode}return false}}
	
	if(AW.gecko){
		AW.getLeft=function(element){var doc=document.getBoxObjectFor(document.body);return document.getBoxObjectFor(element).screenX - doc.screenX+doc.x};
		AW.getTop=function(element){
			var doc=document.getBoxObjectFor(document.body);
			return document.getBoxObjectFor(element).screenY - doc.screenY+doc.y}}
	
	if(AW.ff3){
		AW.getLeft=function(element){return element.getBoundingClientRect().left - document.documentElement.getBoundingClientRect().left};
		AW.getTop=function(element){return element.getBoundingClientRect().top - document.documentElement.getBoundingClientRect().top}}
	
	if(AW.webkit || AW.opera){AW.setOuterHTML=function(element,html){element.outerHTML=html}}})();

(function(){
				AW._addMouseEvents=function(obj,name){
				function clear(){var e=this.element();
				
				if(e){e.className=e.className.replace(/ aw-mouse(over|down)-\w+/g,"")}e=null}
				if(obj.setController){
						if(name){
								obj.setController("highlight",{
									onControlMouseOver:function(){
								 			if(this._controlDisabled){return}var e=this.element();
											if(e){e.className+=" aw-mouseover-"+name}e=null},
											onControlMouseDown:function(){
											if(this._controlDisabled){return}var e=this.element();
											if(e){e.className+=" aw-mousedown-"+name}e=null},
								onControlMouseOut:clear,onControlMouseUp:clear})}}
								else{obj._raiseEvents=true;
								obj.onMouseOver=function(){
									if(this.$owner && this.$owner._controlDisabled){
										return}var e=this.element();
										if(e){e.className+=" aw-mouseover-"+this.$name+(name?" aw-mouseover-"+name:"");
										if(AW.ie){var h=e.offsetHeight}}e=null};
										obj.onMouseDown=function(){
												if(this.$owner && this.$owner._controlDisabled){
												return}var e=this.element();
												if(e){e.className+=" aw-mousedown-"+this.$name+(name?" aw-mousedown-"+name:"");
												if(AW.ie){var h=e.offsetHeight}}e=null};
												...
												AW._startEventManager=function(){
														...
														function keydown(event){
																	...
																	if(event.keyCode >=48 && event.keyCode <=90){key=String.fromCharCode(event.keyCode)}
																	if(key){
														...
														function keyup(){AW._scrollWait=false}
														...
														function raiseKbEvent(name,event){
																try{
																		...
																		obj.raiseEvent(name,event);e=obj.element()}}e=e.parentNode}}
																	catch(err){}}
														...
														function raiseControlEvent(name,obj,event)
														{
															...
															if(obj.setController)
															{
																	...
																	if(name.match("MouseDown")&& event.target.tagName !="INPUT" && event.target.tagName !="SELECT" &&!AW.safari2){event.preventDefault()}
																	if(name=="Clicking")
																	{
																		if(obj.getId()!=AW._edit){obj.focus()}event.done=true}}
																		if((!obj.$active || AW._edit)&&(name.indexOf("Click")> -1)){return}
															...
															if(obj.$name && obj.raiseEvent){
																		...
																		{
																			if(name.match("MouseDown")&& event.target.tagName !="INPUT" && event.target.tagName !="SELECT" && !AW.safari2){event.preventDefault()}
																			if(name=="Clicking"){if(obj.getId()!=AW._edit && obj.$owner.$name !="popup"){obj.$owner.focus()}event.done=true}}
																			if(!obj._raiseEvents){return}if(!AW.$popup &&(!obj.$owner.$active || AW._edit)&&(name.indexOf("Click")> -1)){return}name0="on"+name;
																			if(typeof obj[name0]=="function"){obj[name0](event)}name1=AW.camelCase("on",obj.$name,name);
														...
														{
															try{
																if(AW.ignoreMouse){return}
																var i,obj,temp={};
																while(e){
																	if(e.id && typeof(e.id)=="string"){obj=AW.object(e.id,true);
																	if(obj){e=obj.element();temp[e.id]=true}}e=e.parentNode}
																	for(i in targets)
																	{
																			if(!temp[i]){obj=AW.object(i,true);
																			if(obj){raiseControlEvent("MouseOut",obj,event)}}}
														...
														{
															if(AW.ie){return document.createEventObject(e)}
															else if(AW.webkit || AW.opera || AW.konqueror){
															return{
																	type:e.type,
																	ctrlKey:e.ctrlKey,
																	altKey:e.altKey,
																	...
																	target:e.target,
																	srcElement:e.target}}
															else{	var event=document.createEvent("MouseEvents");
																	event.initMouseEvent(type || 
																		e.type,true,true,e.view,1,e.screenX,
																		e.screenY,e.clientX,e.clientY,e.ctrlKey,
																		e.altKey,e.shiftKey,e.metaKey,e.button,null);
																	event.srcElement=e.target;return event}
															}
																	
															var clickingEvent=null;
																			
															function raiseClickingEvent(){if(clickingEvent){mouseClicks("Clicking")(clickingEvent);clickingEvent=null}}
																			
															var mouseDownId="";
																			
															function mouseClicks(name){return function(event)
															{
																	try{if(name=="MouseUp" && clickingEvent)
																	{
																			raiseClickingEvent()}
																			var target=event.srcElement || event.target;
																			var inside=false;
																			...
																			while(e)
																			{
																				...
																				{
																					obj=AW.object(e.id,true);
																					if(obj){raiseControlEvent(s,obj,event);e=obj.element();inside=true}
																				}
																				e=e.parentNode
																			}
																			...
																			if(name=="MouseDown"){mouseDownId=""}
																			if((name=="MouseDown" || name=="Clicking")&& inside &&(target !==AW.element(target.id))){mouseDownId=target.id}
																			if(name=="MouseUp" && mouseDownId &&(mouseDownId==target.id)){
																					if(AW.ie){event.srcElement.fireEvent("onclick",copyEvent(event))}
																					else{event.target.dispatchEvent(copyEvent(event,"click"))}}
															...
															function focusIE(name){
																	return function(event){
																			try{
																				if(event.srcElement.document !==document){return}
																				if(name=="Deactivating" && AW._edit)
																				{
																					if(!AW._endEdit()){return false}}
																					var obj,e=AW.srcElement(event);
																					while(e){if(e.id && typeof(e.id)=="string")
																					{
																						obj=AW.object(e.id,true);
																						if(obj && obj.setController){
																						switch(name){
																							case "Deactivating":
																												if(!obj.element().contains(event.toElement)){if(AW.ie5){obj._deactivate=true}if(raiseControlEvent(name,obj,event)){event.returnValue=false}}
																												else if(AW.ie5){obj._deactivate=false}break;case "Activating":if(!obj.$active){if(raiseControlEvent(name,obj,event)){event.returnValue=false}}
																												break;
																							case "Deactivated":
																												if(!obj.element().contains(event.toElement)){if(AW.ie5 && !obj._deactivate){break}obj.$active=false;
																												if(raiseControlEvent(name,obj,event)){event.returnValue=false}}break;
																							case "Activated":
																												if(!obj.$active){obj.$active=true;
																												if(raiseControlEvent(name,obj,event)){event.returnValue=false}}break}e=obj.element()}}e=e.parentNode}}catch(err){}}}
																												var handlers={
																												onkeypress:keypress,
																												onkeydown:keydown,
																												onkeyup:keyup,
...
(function(){var obj;var originalText;var originalNode;AW._startEdit=function(target,startText){obj=target;if(!obj.element()|| raiseEvent("editStarting")){return false}originalText=obj.getControlProperty("text");obj.element().className+=" aw-edit-"+(obj.setController?"control":obj.$name);var e=obj.getContent("box/text").element();if(!e || !e.tagName.match(/input/i)){originalNode=e;e=document.createElement("input");e.setAttribute("id",originalNode?originalNode.id:obj.getId()+"-box-edit");e.setAttribute("type","text");e.setAttribute("class",originalNode?originalNode.className:"aw-item-text");e.setAttribute("autocomplete","off");e.setAttribute("value",originalText);e.style.width=calcWidth(originalNode);if(originalNode){if(AW.ie){originalNode.parentNode.insertBefore(e,originalNode);e.focus();e.parentNode.removeChild(originalNode)}else{originalNode.parentNode.replaceChild(e,originalNode);e.focus()}}else{var parent=obj.element();parent.innerHTML="<span class=\"aw-item-ruler\"></span>";parent.appendChild(e);e.focus()}e.parentNode.scrollTop=0}if(AW.ie){if(!AW.ms8){e.setExpression("aw-value","this.value")}e.attachEvent("oncontextmenu",nobubble);e.attachEvent("onselectstart",nobubble);e.attachEvent("onpropertychange",oninput);e.attachEvent("onkeydown",onkeydown);e.attachEvent("onbeforedeactivate",onbeforedeactivate)}else{e.addEventListener("contextmenu",nobubble,false);e.addEventListener("input",oninput,false);e.addEventListener("keydown",onkeydown,false);obj.element().addEventListener("mousedown",onmousedown,true)}if(obj.$owner){obj.$owner.$edit=true}AW._edit=obj.getId();raiseEvent("editStarted");if(typeof(startText)=="string"){obj.setControlProperty("text",startText);e.value=startText}else{e.select()}e=null;return true};AW._endEdit=function(){if(AW._endEditFlag){return false}AW._endEditFlag=true;try{if(originalText !=obj.getControlProperty("text")&& !AW._commitEdit()){return false}if(raiseEvent("editEnding")){return false}if(AW.$popup){AW.$popup.hidePopup()}var e=obj.element().getElementsByTagName("INPUT")[0];if(AW.ie){if(!AW.ms8){e.removeExpression("aw-value")}e.detachEvent("onselectstart",nobubble);e.detachEvent("oncontextmenu",nobubble);e.detachEvent("onpropertychange",oninput);e.detachEvent("onkeydown",onkeydown);e.detachEvent("onbeforedeactivate",onbeforedeactivate)}else{e.removeEventListener("contextmenu",nobubble,false);e.removeEventListener("input",oninput,false);e.removeEventListener("keydown",onkeydown,false);obj.element().removeEventListener("mousedown",onmousedown,true)}e.parentNode.scrollLeft=0;var text=obj.getControlProperty("text");if(originalNode){originalNode.innerHTML=text;if(AW.ie){e.parentNode.insertBefore(originalNode,e);removeElement(e)}else{e.parentNode.replaceChild(originalNode,e)}originalNode=null}else if(!obj.getContent("box/text").element()){if(AW.ie){e.parentNode.removeChild(e.previousSibling);e.insertAdjacentHTML("beforeBegin",text);removeElement(e)}else{e.parentNode.innerHTML=text}}e=null;obj.element().className=obj.element().className.replace(/ aw-edit-\w+/ig,"");if(obj.$owner){obj.$owner.$edit=false}AW._edit="";raiseEvent("editEnded");return true}finally{AW._endEditFlag=false}};AW._commitEdit=function(){if(!AW._edit){return false}if(raiseEvent("validating")){return false}originalText=obj.getControlProperty("text");raiseEvent("validated");return true};AW._cancelEdit=function(){if(!AW._edit){return false}obj.setControlProperty("text",originalText);obj.element().getElementsByTagName("INPUT")[0].value=originalText;return true};AW._updateEdit=function(){obj.refreshClasses();obj.element().className+=" aw-edit-"+obj.$name;var e=obj.element().getElementsByTagName("INPUT")[0];var text=obj.getControlProperty("text");if(e && e.value !=text){if(AW.ie){var r=document.selection.createRange();r.collapse();r.select()}e.value=text}e=null};function raiseEvent(name){var item=obj.setController?"control":obj.$name;var fullname=AW.camelCase("on",item,name);var text=obj.getControlProperty("text");return obj.raiseEvent(fullname,text,obj.$0,obj.$1,obj.$2)}function nobubble(event){if(AW.ie){event.cancelBubble=true}else{event.stopPropagation()}}function oninput(event){var text1=obj.getControlProperty("text");var text2=(event.srcElement||event.target).value;if(text2 !=text1){obj.setControlProperty("text",text2)}var text3=obj.getControlProperty("text");if(text3 !=text2){(event.srcElement||event.target).value=text3}}function onkeydown(event){if(AW.ie){var r=event.srcElement.createTextRange();var s=document.selection.createRange();if((event.keyCode==36 || event.keyCode==37)&&(r.compareEndPoints("StartToEnd",s)||r.compareEndPoints("StartToStart",s))){event.cancelBubble=true;r=null;s=null;return}if((event.keyCode==35 || event.keyCode==39)&&(r.compareEndPoints("EndToEnd",s)|| r.compareEndPoints("EndToStart",s))){event.cancelBubble=true;r=null;s=null;return}}else{if((event.keyCode==36 || event.keyCode==37)&& event.target.selectionEnd > 0){event.stopPropagation();return}if((event.keyCode==35 || event.keyCode==39)&& event.target.selectionStart < event.target.value.length){event.stopPropagation();return}}}function onbeforedeactivate(event){if(obj.element().contains(event.toElement)){event.returnValue=false;event.cancelBubble=true}}function onmousedown(event){if(event.target && event.target.tagName !="INPUT"){event.preventDefault();return}}function calcWidth(e){if(!e){return "100%"}var w=e.offsetWidth+e.parentNode.clientWidth - 5;var i,ee=e.parentNode.childNodes;for(i=0;i<ee.length;i++){w -=(ee[i].offsetWidth+1)}return w+"px"}function removeElement(node){document.selection.empty();node.id="aw-edit";window.setTimeout(function(){try{node.parentElement.removeChild(node)}catch(err){}node=null},0)}})();
	 ...
	 try{
		 if(arguments.length==1)
		 {
			 ...
			 if(typeof name=="object"){
				 for(var i in name){if(typeof(i)=="string"){
					 this.setContent(i,name[i])}}}
			 else{this.setContent("html",name)}}
		 else{
			 var split=name.match(/^(\w+)\W(.+)$/);
			 if(split){
				 var ref=this.getContent(split[1]);
				 ref.setContent(split[2],value)}
			 else{var param="_"+name+"Content";
			 if(this[param]==null){this._content+=" "+name}
			 if(value && typeof value=="object"){value._parent=this;
			 if(!value.defineModel){value._id=this._id+"-"+name}}
			 this[param]=value;
			 this._innerHTML="";resetHTMLCache(this)}}}
	 catch(error){this.handle(error)}};
	 
	 obj.refreshContents=function()
	 {
		 try{var element=this.element();
		 if(!element || element.tagName.match(/input|textarea/i)){return}
		 if(AW._edit && AW.contains(element,AW.element(AW._edit))){return}
		 var i,s="",content=this._content.split(" ");
		 for(i=1;i<content.length;i++){s+=this.getContent(content[i])}
		 try{
			 var focus=AW.element(AW._focus)}
		 catch(err){}element.innerHTML=s;
		 
		 try{if(focus !==AW.element(AW._focus)){AW.element(AW._focus).focus()}}
		 catch(err){}}
		 catch(error){this.handle(error)}};
		 ...
		 var getParamStr=function(i){
			 ...
			 var getControlFunc=function(v){
				 return function(){return v}};obj.innerHTML=function(){
					 try{if(this._innerHTML){return this._innerHTML}
					 this._innerParamLength=0;
					 var i,j,name,value,param,param1,param2,html,item,s="";
					 var content=this._content.split(" ");
					 for(i=1;i<content.length;i++){name=content[i];
					 value=this["_"+name+"Content"];
					 
					 if(typeof(value)=="function"){
						 param=getParamStr(this._innerParamLength++);
						 this[param]=value;s+=param}
					 else if(typeof(value)=="object" && value.defineModel){
						 param=getParamStr(this._innerParamLength++);
						 this[param]=getControlFunc(value);s+=param}
					 else if(typeof(value)=="object"){
						 item=value;html=item.outerHTML().replace(/\{id\}/g,"{id}-"+name);
					 ...
					 obj.outerHTML=function(){
						 try{
							 ...
							 if(!this._tag){return innerHTML}var i,tmp,name,value,param;
							 var html="<"+this._tag+" id=\"{id}\"";tmp="";
							 var classes=this._classes.split(" ");
							 for(i=1;i<classes.length;i++){name=classes[i];
							 value=this["_"+name+"Class"];
							 if(typeof(value)=="function"){param=getParamStr(this._outerParamLength++);
							 this[param]=value;value=param}if(value || value===0 || value===false){tmp+="aw-"+name+"-"+value+" "}}
							 if(tmp){html+=" class=\""+tmp+"\""}tmp="";var styles=this._styles.split(" ");
							 for(i=1;i<styles.length;i++){name=styles[i];value=this["_"+name+"Style"];
							 if(typeof(value)=="function"){param=getParamStr(this._outerParamLength++);
							 this[param]=value;value=param}tmp+=name+":"+value+";"}if(tmp){html+=" style=\""+tmp+"\""}tmp="";
							 var attributes=this._attributes.split(" ");
							 for(i=1;i<attributes.length;i++){name=attributes[i];
							 value=this["_"+name+"Attribute"];if(typeof(value)=="function"){param=getParamStr(this._outerParamLength++);
							 this[param]=value;value=param}
							 else if(specialAttributes[name] && !value){value=null}if(value !==null){tmp+=" "+name+"=\""+value+"\""}}html+=tmp;
							 tmp="";var events=this._events.split(" ");
							 for(i=1;i<events.length;i++){name=events[i];
							 value=this["_"+name+"Event"];
							 if(typeof(value)=="function"){
								 value="AW(this,event)"}tmp+=" "+name+"=\""+value+"\""}html+=tmp;html+=">"+innerHTML+"</"+this._tag+">";
								 this._outerHTML=html;return html
						 }
						 catch(error){this.handle(error)}};
						 ...
						 obj.toString=function(){
								 try{
									 ...
									 if(param_cache.length < max){
										 ...
										 if(value===null){value="";param=specialParams[i];
						 ...
						 function getParent(obj){
							 while(obj){if(!obj._parent || obj.defineModel){
								 return obj}obj=obj._parent}}
						 ...
						 obj.refresh=function()
						 {
							 ...
							 if(element){
								 ...
								 try{var focus=AW.element(AW._focus)}
								 catch(err){}AW.setOuterHTML(element,this.toString());
								 
								 try{if(focus !==AW.element(AW._focus)){AW.element(AW._focus).focus()}}
								 catch(err){}}}catch(error){this.handle(error)}};
								 
								 obj.setSize=function(width,height){
									 if(typeof(width)!="undefined"){this.setStyle("width",width - AW.dx+"px")}
									 if(typeof(height)!="undefined"){this.setStyle("height",height - AW.dy+"px")}};
								 
								 obj.setPosition=function(left,top){this.setStyle("position","absolute");
 ...
 obj._endUpdate=function(){
	 ...
	 for(i=0;i<u.length;i++){r=u[i];
	 this.raiseEvent(r.e1,r.v,r.a,r.b,r.c)}
	 for(i=0;i<u.length;i++){r=u[i];r.f.call(this,r.v,r.a,r.b,r.c)}
	 for(i=0;i<u.length;i++){r=u[i];
	 ...
	 obj.defineTemplate=function(name,template){
		 ...
		 this[get]=function(a,b,c){
			 var r,id,clone;
			 if(a===undef){id=this._id+name1;r=this[ref]}
			 else if(b===undef){id=this._id+name2+a;r=this[ref1] && this[ref1][a];
			 if(!r){r=this[ref];clone=true}}
			 else if(c===undef){
				 id=this._id+name2+a+"-"+b;r=this[ref2] && this[ref2][a] && this[ref2][a][b];if(!r){r=(this[ref1] && this[ref1][a])|| this[ref];clone=true}}
			 else{id=this._id+name2+a+"-"+b+"-"+c;r=this[ref3] && this[ref3][a] && this[ref3][a][b] && this[ref3][a][b][c];if(!r){r=(this[ref2] && this[ref2][a] && this[ref2][a][b])||(this[ref1] && this[ref1][a])|| this[ref];clone=true}}
			 if(typeof(r)=="function"){
				 return r.call(this,a,b,c)}
			 if((this.$clone)&&(clone || r.$owner !=this)){r=r.clone()}r.$owner=this;r.$0=a;r.$1=b;r.$2=c;r._id=id;return r};
			 templates[get]=function(a,b,c){if(a===undef){return this.$owner[get](this.$0,this.$1,this.$2)}
			 if(b===undef){return this.$owner[get](a,this.$0,this.$1)}if(c===undef){return this.$owner[get](a,b,this.$0)}return this.$owner[get](a,b,c)};
			 this[set]=function(template,a,b,c){var previous;
			 if(a===undef){previous=this[ref];this[ref]=template}
			 else if(b===undef){if(!this[ref1]){this[ref1]={}}
			 previous=this[ref1][a];this[ref1][a]=template}
			 else if(c===undef){if(!this[ref2]){this[ref2]={}}
			 if(!this[ref2][a]){this[ref2][a]={}}
			 previous=this[ref2][a][b];
			 this[ref2][a][b]=template}
			 else{if(!this[ref3]){this[ref3]={}}
			 if(!this[ref3][a]){this[ref3][a]={}}
			 if(!this[ref3][a][b]){this[ref3][a][b]={}}previous=this[ref3][a][b][c];
			 this[ref3][a][b][c]=template}
			 
			 if(template){
				 template.$name=name;template.$0=a;template.$1=b;
				 template.$2=c;
				 ...
				 this[get1]=this[get];
				 this[set1]=this[set];
				 templates[get1]=templates[get]};obj.$clone=true;
				 ...
				 obj.onControlVisibleChanged=function(value){this.setClass("visible",value)};
				 obj.focus=function(){};
				 obj.setName=function(name){
					 var hidden=new AW.HTML.INPUT;hidden.setAttribute("type","hidden");
					 hidden.setAttribute("name",name);
					 hidden.setAttribute("value",function(){return AW.valueToText(this.getControlData())});
					this.setContent("data",hidden)};
					
				obj.setController=function(name,controller){
					var i,n="_"+name+"Controller";
					this[n]=controller;
					for(i=0;i<this._controllers.length;i++){if(this._controllers[i]==n){return}}
					...
					obj.raiseEvent=function(name,source,a,b,c){
						...
						if(typeof(handler)=="function"){
							r=handler.call(this,source,a,b,c);if(r){return r}}
						for(i=0;i<this._controllers.length;i++){
							...
							if(typeof(handler)=="function"){
								...
								r=this.raiseEvent(handler,source,a,b,c);if(r){return r}}}};
								
					obj.action=function(name,source,a,b,c){this.raiseEvent(AW.camelCase("on",name),source,a,b,c)};
					obj.$active=false;
 ...
 AW.Templates.ImageText=AW.System.Template.subclass();AW.Templates.ImageText.create=function(){var obj=this.prototype;function _image(){return this.getControlProperty("image")|| "none"}function _text(){return this.getControlProperty("text")}function _tooltip(){return this.getControlProperty("tooltip")}var span=AW.HTML.SPAN;var image=new span;image.setClass("item","image");image.setClass("image",_image);var ruler=new span;ruler.setClass("item","ruler");var text=new span;text.setClass("item","text");
 text.setContent("html",_text);var box=new span;box.setClass("item","box");box.setContent("sign","");box.setContent("marker","");box.setContent("image",image);box.setContent("ruler",ruler);box.setContent("text",text);obj.setTag("span");obj.setClass("item","template");obj.setClass("templates","imagetext");obj.setAttribute("title",_tooltip);obj.setContent("box",box);obj.refresh=function(){this.refreshStyles();this.getContent("box/image").refreshClasses();if(AW._edit==this.getId()){AW._updateEdit()}else{this.refreshClasses();this.getContent("box/text").refreshContents()}};AW._addMouseEvents(obj);if(AW.gecko){obj.setAttribute("tabIndex","-1")}obj.startEdit=function(text){AW._startEdit(this,text)}};
 
 AW.Templates.Image=AW.Templates.ImageText.subclass();AW.Templates.Image.create=function(){var obj=this.prototype;obj.setClass("templates","image");obj.getContent("box").setTag("");obj.getContent("box/text").setTag("");
 obj.setContent("box/text/html","");obj.startEdit=null};
 ...
 AW.Templates.CheckBox=AW.Templates.ImageText.subclass();AW.Templates.CheckBox.create=function(){var obj=this.prototype;obj.setClass("value",function(){return this.getControlProperty("value")|| false});obj.setClass("toggle","checkbox");obj.setClass("templates","checkbox");var marker=new AW.HTML.SPAN;marker.setClass("item","marker");
 obj.setContent("box/marker",marker);obj.setEvent("onclick",function(){var value=this.getControlProperty("value");this.setControlProperty("value",!value)});obj.startEdit=null;AW._addMouseEvents(obj,"toggle")};AW.Templates.Checkbox=AW.Templates.CheckBox;
 ...
 box.setContent("html",function(){return this.getLayout()});obj.setContent("box",box)};

 AW.Templates.List=AW.System.Template.subclass();AW.Templates.List.create=function(){var obj=this.prototype;obj.setTag("span");obj.setClass("templates","list");
 ...
 AW.Panels.Vertical=AW.System.Template.subclass();AW.Panels.Vertical.create=function(){var obj=this.prototype;obj.setClass("vpanel","template");var span=AW.HTML.SPAN;var box=new span;var left=new span;var center=new span;var right=new span;box.setClass("vpanel","box");left.setClass("vpanel","left");center.setClass("vpanel","center");right.setClass("vpanel","right");
 left.setContent("html",function(){return this.getPanel(0)});center.setContent("html",function(){return this.getPanel(1)});right.setContent("html",function(){return this.getPanel(2)});box.setContent("left",left);box.setContent("center",center);box.setContent("right",right);obj.setContent("box",box)};
 
 AW.Panels.Grid=AW.System.Template.subclass();
 
 AW.Panels.Grid.create=function()
 {
	 var obj=this.prototype;obj.setClass("gpanel","template");
	 var span=AW.HTML.SPAN;var box=new span;
	 var top=new span;
	 var middle=new span;
	 var bottom=new span;
	 ...
	 function panel(i,j){return function(){return this.getPanel(i,j)}}
	 ...
	 for(i=0;i<3;i++){
		 ...
		 if(AW.ms6)
		 {
			 center.setStyle("width","expression(this.parentElement.clientWidth-this.previousSibling.offsetWidth-this.nextSibling.offsetWidth)")}rows[i].setContent("left",left);
	...
	obj.changeScrollLeft=function(x){
		...
		obj.changeScrollTop=function(y){var e1=this.getContent("box/middle/center").element();
		...
		obj.changePanelWidth=function(width,part){
			...
			if(part=="left"){
				for(i=0;i<3;i++){
					...
					if(AW.ie && !AW.strict){r.setStyle("padding-left",w)}
					else{r.getContent("center").setStyle("left",w)}}
				}
			else if(part=="right"){
				for(i=0;i<3;i++){
					r=this.getContent(parts[i]);
					...
					if(AW.ie && !AW.strict){r.setStyle("padding-right",w)}
					else{r.getContent("center").setStyle("right",w)}}}
			};
			obj.changePanelHeight=function(height,part){
				...
				if(part=="top"){
					...
					else{this.getContent("box/middle").setStyle("top",h)}
					}
				else if(part=="bottom"){
					this.getContent("box/bottom").setStyle("height",h);
					this.getContent("box/bottom").setStyle("display",height?"block":"none");
					if(AW.ie && !AW.strict){this.getContent("box").setStyle("padding-bottom",h)}
					else{this.getContent("box/middle").setStyle("bottom",h)}}
				};
 ...
 AW.UI.Controllers.Actions=(function(){
	 ...
	 function last(){
		 ...
		 var a=this.getViewIndices();return a?a[p]:p}
	 function next(){
		 ...
		 return function(event,index){
			 ...
			 if(event && event.type=="mousedown"){
				 this.setTimeout(function(){
					 if(this.$active){action.call(this,i)}})}
			 else{if(this.$active){action.call(this,i)}}event=null}}
 ...
 AW.UI.ImageText.create=function(){
	 ...
	 var obj=this.prototype;var _super=this.superclass.prototype;
	 obj.setClass("templates","");obj.setClass("ui","imagetext");
	 obj.setClass("item","control");obj.setClass("text","expand");
	 function _tabIndex(){return this.getTabProperty("index")}if(AW.ie || AW.gecko){obj.getContent("box/text").setAttribute("tabIndex",_tabIndex)}
	 obj.refresh=function(){if(this.getContent("box/text").element()){this.refreshClasses();this.refreshStyles();this.refreshAttributes();
 ...
 if(this.getContent("data")){this.getContent("data").refreshAttributes()}}
	 else{_super.refresh.call(this)}};var _refresh=function(){this.refresh()};
	 var itemController={
	 ...
	 obj.setEvent("onactivate",function(event){var e=this.getContent("box/text").element();
	 if(e && event.srcElement !=e){this.setTimeout(function(){if(this.$active){e.setActive()}})}});
	 if(AW.webkit || AW.opera || AW.konqueror){var focus=new AW.HTML.SPAN;focus.setTag("a");
	 focus.setClass("control","focus");focus.setAttribute("tabIndex",function(){return this.getTabProperty("index")});
	 if(AW.webkit){focus.setAttribute("href","#")}if(AW.opera){focus.setContent("html","&nbsp;")}obj.setContent("focus",focus)}obj.focus=function()
	 {try{if(this.getControlDisabled()){return}if(AW.webkit || AW.opera || AW.konqueror){this.getContent("focus").element().focus();
	 return}this.getContent("box/text").element().focus()}catch(err){}};
	 obj.onControlDisabledChanged=function(value){this.setClass("disabled",value?"control":null);
	 this.setAttribute("disabled",value?true:null);this.getContent("box/text").setAttribute("disabled",value?true:null)};
	 obj.setSize=function(width,height){if(typeof(width)!="undefined"){
		 if(AW.ms5 || AW.ms6 || this._textClass !="expand"){this.setStyle("width",width - AW.dx+"px")}
		 else{if(AW.ff2 || AW.ff15 || AW.ff1){this.setStyle("min-width",width+"px")}
		 else if(AW.opera && opera.version()< 9.5 && this._uiClass=="button"){this.setStyle("min-width",width - 8+"px")}
		 else{this.setStyle("min-width",width - AW.dx+"px")}if(AW.ms7 || AW.ms8){this.getContent("box").setStyle("min-width",width - AW.dx+"px")}}}
	 if(typeof(height)!="undefined"){this.setStyle("height",height - AW.dy+"px")}}};
 ...
 /******************************************************************************************/
 //					Construcción de los objetos de la grid
 /******************************************************************************************/
		 ...
		 try{
			 ...
			 this.setTimeout(function(){
				 ...
				 if(e && !document.title.match(/ivewid/i)&&(new Date).getMilliseconds()< 300){
					 ...
					 //Licencia encontrada Retirar
					 
					 //AW.setOuterHTML(e,"s<ap ntsly=ed\"silpyab:olkcp;sotioi:nbaosuletw;diht8:%0h;iehg:t02xpl;fe:t01;%obttmo2:p5;xobdrre1:xps lodib;cagkornu:df#ffc;loro#:0f;0ettxa-ilngc:neet\"rA>tcviWediegst2 5.- E avultaoi neVsroi.n/<psna>".replace(/(.)(.)/g,"$2$1"))}});
					 
					 AW.setOuterHTML(e,"".replace(/(.)(.)/g,"$2$1"))}});
 ...
 AW.Grid.Controllers.Cell=(function(){
	 ...
	 function tooltip(event,col,row){
		 ...
		 if(e){e.setAttribute("title",s)}e=null}
	 
	 function calcState(v,i,j){
		 this.calculateCellState(i,j)}
	 
	 return{onCellMouseOver:tooltip,onCellSelectedChanged:calcState,onCellDataChanged:cellData,onCellTextChanged:refresh,onCellLinkChanged:refresh,onCellImageChanged:refresh,onCellValueChanged:refresh,onCellStateChanged:refreshClasses}})();

 AW.Grid.Controllers.Edit=(
		 function(){
			 function startEdit(event){
				 if(AW._edit){return}var s;
				 if(event && event.type=="keypress"){
					 if(event.keyCode==27 || event.keyCode==13){return}
					 s=String.fromCharCode(event.keyCode || event.charCode)}
				 if(this.startCellEdit(s)){AW.setReturnValue(event,false)}}
			 
			 function startOrEnd(event){
				 if(!AW._edit){this.startCellEdit()}
				 else{this.endCellEdit()}
				 AW.setReturnValue(event,false)}
 ...
 AW.Grid.Controllers.Row=(function(){
	 function refresh(value,row){this.getRow(row).refreshClasses()}
	 function addRow(row){
		 ...
		 if(!this.$extended){
			 ...
			 if(e){AW.setOuterHTML(e,this.getRow(prev).toString()+this.getRow(row).toString())}e=null}
		 else{
			 for(i=0;i<3;i++){
				 e=this.getRow(prev,i).element();
				 if(e){AW.setOuterHTML(e,this.getRow(prev,i).toString()+this.getRow(row,i).toString())}e=null}
			 }
 ...
 AW.Grid.Controllers.View={
	 onRowIndicesChanged:function(indices){
	 	var positions=[];
	 	for(var i=0;i<indices.length;i++){positions[indices[i]]=i}
	 	this.setRowPosition(positions);
	 	this.refresh()} ,
	 onColumnIndicesChanged:function(indices){
		 var positions=[];
		 for(var i=0;i<indices.length;i++){positions[indices[i]]=i}
		 this.setColumnPosition(positions);this.setColumnCount(indices.length);this.refresh()}};
 ...
 AW.Grid.Controllers.Navigation=(function(){
	 ...
	 function syncSelected(){
		 ...
		 var s=serial;
		 if(!this._columnsSelected || this._columnsSelected.$ !==this){this._columnsSelected={$:this}}
		 if(!this._rowsSelected || this._rowsSelected.$ !==this){this._rowsSelected={$:this}}
		 ...
		 if(mode=="cell"){
			 ...
			 for(i=0;i<cols.length;i++){col=cols[i];(cc[col]?unchanged:added).push(col);cc[col]=s}
			 for(col in cc){if(cc[col] && cc[col] !==s && !ext[col]){delete cc[col];removed.push(col)}}
			 for(i=0;i<rows.length;i++){
				 ...
				 if(!rr[row]){
					 ...
					 for(j=0;j<added.length;j++){
						 col=added[j];
						 this.setCellSelected(true,col,row)}}
				 else{rr[row]=s;for(j=0;j<added.length;j++){
					 col=added[j];
					 this.setCellSelected(true,col,row)}
	 ...
	 return{
		 ...
		 onSelectionModeChanged:function(mode){
		 	...
		 	case "single-cell":
		 						...
		 						this.setCurrentSelection("cell");break;
		 	case "single-row":
		 						this.setController("selection",Grid.SingleRow);
		 						this.setController("edit",{});
		 						this.setCurrentSelection("row");break;
		 	case "multi-cell":
 ...
 AW.Grid.Controllers.Content={
		 onScrollLeftChanged:function(x){this.getView().changeScrollLeft(x)},
		 onScrollTopChanged:function(y){this.getView().changeScrollTop(y)},
		 onContentWidthChanged:function(width,panel){this.getView().changePanelWidth(width,panel)},
		 onContentHeightChanged:function(height,panel){this.getView().changePanelHeight(height,panel)}};
 
 AW.Grid.Controllers.Virtual=(function(){
	 var oldScrollTop,oldScrollLeft,scrollTime=new Date();
	 function calcVirtual(dir){if(!this.getVirtualMode()){return}dir=dir || 0;
	 var scrollTop=this.getScrollTop();
	 var scrollLeft=this.getScrollLeft();
	 var totalHeight=this.getContentHeight("center");
	 var totalWidth=this.getContentWidth("center");
	 if(!totalHeight || !totalWidth){return}
	 var delta1=dir < 0?10:1;var delta2=dir > 0?10:1;
	 ...
	 var rvScroll=start * rowHeight;var rvOffset=start+rowOffset;
				 ...
				 onScrollTopChanging:function(){
					 ...
					 onScrollLeftChanged:function(scrollLeft){
						 if(this.getVirtualMode()&& scrollLeft !=oldScrollLeft){
							 AW._scrollWait=true;startVirtual.call(this)}},
							 onScrollTopChanged:function(scrollTop){
								 if(this.getVirtualMode()&& scrollTop !=oldScrollTop){
									 AW._scrollWait=true;
									 if(Math.abs(scrollTop - oldScrollTop)> this.getScrollHeight()/5){AW._scrollWait=false}
									 var dir=scrollTop > oldScrollTop?1:-1;startVirtual.call(this,dir)}
								 },
							 onCurrentRowChanged:function(i){
									 ...
									 if(!max){return}
									 if(top < scroll){this.setScrollTop(top)}
									 if(max+scroll < bottom){this.setScrollTop(bottom - max)}
									 if(AW.ie && this.element()){
										 ...
										 onCurrentColumnChanged:function(index){
											 ...
											 if(!this.$extended){mw=lw;lw=0}
											 for(i=0;i<lc;i++){lw+=this.getColumnWidth(a?a[i]:i)}
											 for(i=lc;i<Math.min(col,c-rc-1);i++){mw+=this.getColumnWidth(a?a[i]:i)}
											 for(i=c-rc;i<c;i++){rw+=this.getColumnWidth(a?a[i]:i)}
											 if(!col){mw=0}if(mw < scroll){this.setScrollLeft(mw);return}
											 var right=mw+this.getColumnWidth(index);
											 if(max+scroll < right){this.setScrollLeft(right - max)}}}})();
 ...
 AW.Grid.Controllers.Grid={
	 		...
	 		onHeadersTemplateChanged:function(headers){
	 					...
	 					headers.mapTemplate("item",function(i){return this.$owner.getHeader(i,this.$0)+this.$owner.getSeparator(i,this.$0)});
	 					headers.mapTemplate("selector",function(){return this.$owner.getTopSelector(this.$0)+(this.$owner.getSelectorResizable()&& !this.$0?this.$owner.getSeparator("selector"):"")});
	 					headers.mapModel("view","column");headers._raiseEvents=false},
 ...
 AW.Grid.Header.create=function(){
	 ...
	 function _direction(){return this.getSortProperty("direction")|| "none"}
	 obj.setClass("sort",_direction);
	 var sort=new AW.HTML.SPAN;sort.setClass("grid","sort");
	 obj.setContent("box/text/sort",sort);
	 
	 obj.element=function()
	 {
		 if(typeof(this.$1)=="undefined" && this.$owner && this.$owner.$extended){return _super.element.call(this.$owner.getHeader(this.$0,0))}
		 else{return _super.element.call(this)}}
 };
 ...
 AW.Grid.Control.create=function()
 {
 			...
 			var focus=new AW.HTML.TEXTAREA;focus.setClass("control","focus");
 			focus.setAttribute("tabIndex",function(){return this.getTabProperty("index")});
 			var sample=new AW.HTML.SPAN;sample.setClass("row","sample");
 			sample.setClass("grid","row");
 			var box=new AW.HTML.SPAN;box.setClass("grid","box");
 			...
 			function position(i){return Number(i)}
 			...
 			obj.getLastColumn=function(){
 				...
 				var a=this.getColumnIndices();return a?a[p]:p};
 			...
 			obj.getLastRow=function(){
 				...
 				var a=this.getRowIndices();return a?a[p]:p};
 			...
 			obj.selectRow=function(row,mode){
 				if(this.$edit){this.focus()}
 				if(this.$edit){return}
 				this._startUpdate();
 				if(this.getCurrentSelection()!="row"){
 					this.setCurrentSelection("row")}
 				var start,end,i,a;var rr=this.getSelectedRows();
 				if(!mode){if(rr.length !=1 || rr[0] !=row){this.setSelectedRows([row])}}
 				else if(mode==1){if(!rr.length){this.setSelectedRows([row])}
 				else{start=this.getRowPosition(rr[0]);
 				end=this.getRowPosition(row);
 				a=this.getRowIndices();rr=[];
 				if(start < end){for(i=start;i <=end;i++){rr.push(a?a[i]:i)}}
 				else{for(i=start;i >=end;i--){rr.push(a?a[i]:i)}}
 				this.setSelectedRows(rr)}}
 				else if(mode==2){this.setRowSelected(!this.getRowSelected(row),row)}
 				if(row!=this.getCurrentRow()){this.setCurrentRow(row)}this._endUpdate()};
 				
 			obj.selectCell=function(col,row,mode){
 				if(this.$edit){this.focus()}if(this.$edit){return}this._startUpdate();
 				if(this.getCurrentSelection()!="cell"){this.setCurrentSelection("cell")}
 				var start,end,i,a;
 				var cc=this.getSelectedColumns();
 				var rr=this.getSelectedRows();
 				if(!mode){if(cc.length !=1 || cc[0] !=col){this.setSelectedColumns([col])}
 				if(rr.length !=1 || rr[0] !=row){this.setSelectedRows([row])}}
 				else if(mode==1){if(!cc.length){this.setSelectedColumns([col])}
 				else{start=this.getColumnPosition(cc[0]);
 				end=this.getColumnPosition(col);
 				a=this.getColumnIndices();cc=[];
 				
 				if(start < end){for(i=start;i <=end;i++){cc.push(a?a[i]:i)}}
 				else{for(i=start;i >=end;i--){cc.push(a?a[i]:i)}}this.setSelectedColumns(cc)}
 				
 				if(!rr.length){this.setSelectedRows([row])}
 				else{
 				...
 				if(row!=this.getCurrentRow())
 				{
 					...
 					this.setCurrentRow(row)}this._endUpdate()};
 					...
 					obj.calculateCellState=function(i,j){
 						...
 						if(this.getCellSelected(i,j)){state="selected"}this.setCellState(state,i,j)};
 					...
 					obj.startCellEdit=function(text){
 						...
 						var cell=this.getCell(c,r);return AW._startEdit(cell,text)};
 					...
 					box.setEvent("onactivate",function(event){
 						try{
 								if(event.srcElement.tagName=="SPAN")
 								{
 									...
 									if(src && src.setController && src !==this){return}
 									var target=this;
 					...
 					focus.setEvent("onselectstart",function(event){
 							if(AW.ie){event.cancelBubble=true}else{event.stopPropagation()}});
 						
 					focus.setEvent("onbeforecopy",function(event){
 							if(AW.webkit){expandFocus.call(this,false)}});
 					...
 					obj.getSelectedText=function(){
 							...
 							if(this.getCurrentSelection()=="row"){
 								...
 								for(c=0;c<count;c++){cols[c]=indices?indices[c]:c}}
 					...
 					focus.setEvent("oncut",function(event){
 							...
 							if(AW.ie){
 								window.clipboardData.setData("Text",txt);event.returnValue=false}
 							else{
 								var e=this.getContent("box/focus").element();
 								e.value=txt;e.select()}
 							this.startCellEdit("")});
 					...
 					focus.setEvent("onpaste",function(event){
 							...
 							else{return}this.startCellEdit(txt)});
 					...
 					function ffCopy(){
 						...
 						if(AW.gecko){
 								obj.setController("copypaste",{onKeyCtrlC:AW.ffx?ffCopy:null,onCellMouseUp:function(event){
 								if(event.button==2){expandFocus.call(this,true);
 								if(AW.ff3 && !AW._edit){
 					...
 					obj.focus=function(){
							try{
								if(this.getControlDisabled()){
									return}var e=this.getContent("box/focus").element();
									e.value=" ";
									if(AW.opera){
										e.focus()}
									else{e.select()}
 					...
 					obj.deleteRow=function(row){
 								if(this.raiseEvent("onRowDeleting",row)){return}
 								if(this._cellModel && this._cellModel.deleteRow){this._cellModel.deleteRow(row)}
 								var i,count=this.getRowCount();
 								var a=this.getRowIndices();
 								if(!a){a=[];for(i=0;i<count;i++){a[i]=i}i=row}
 								else{i=this.getRowPosition(row)}
 								a.splice(i,1);
 								var refresh=this.refresh;
 								this.refresh=function(){};
 								this.setRowIndices(a);
 					...
 					obj.sort=function(column,direction){this.raiseEvent("doSort",direction,column)};
 ...
 AW.Grid.Extended.create=function(){
	 var obj=this.prototype;obj.$extended=true;
	 ...
	 switch(p){
		 	case "count":
		 		...
		 		if(i=="right"){return right}return 0;
		 	case "offset":
		 		if(i=="left"){return 0}if(i=="center"){return left}
		 		if(i=="right"){return this.$owner.getColumnProperty("count")- right}return 0;
		 	default:
 ...
 AW.XML.Table.create=function(){
	 var obj=this.prototype;obj.response=function(xml){
		 this.setXML(xml);
		 if(this.$owner){
			 ...
			 this.$owner.setRowCount(this.getCount());this.$owner.refresh()}};
			 obj.defineProperty("XML");
			 obj.setXML=function(xml){if(!xml.nodeType){
				 var s=""+xml;xml=new ActiveXObject("MSXML2.DOMDocument");
				 xml.loadXML(s)}xml.setProperty("SelectionLanguage","XPath");
			...
			if(!AW.ie){
				...
				obj.setXML=function(xml){if(!xml.nodeType){
					...
					xml=parser.parseFromString(""+xml,"text/xml")}
						...
						xml.appendChild(node)}namespaces={};
						...
						for(var i=1;i<a.length;i++){
							...
							this._ready=true}}obj.getXML=function(){return this._xml};
							obj._dataPath="*";obj._itemPath="*";obj._valuePath="*";obj._valuesPath=[];
							obj._formats=[];obj.setColumns=function(array){this._valuesPath=array};
							obj.setRows=function(xpath){this._itemPath=xpath};
							obj.setTable=function(xpath){this._dataPath=xpath};
							obj.getCount=function(){if(!this._items){return 0}return AW.ie?this._items.length:this._items.snapshotLength};
							obj.getData=function(i,j){var node=this.getNode(i,j);
							return node?(AW.ie?node.text:node.textContent):""};
							obj.getNode=function(j,i)
							{
								if(!this._items || !this._items[i]){return null}
								if(this._valuesPath[j]){return this._items[i].selectSingleNode(this._valuesPath[j])}
								else{return this._items[i].selectNodes(this._valuePath)[j]}
							};
							if(!AW.ie){obj.getNode=function(c,r){if(!this._items){return null}var row=this._items.snapshotItem(r);if(!row){return null}
							if(this._valuesPath[c]){return row.ownerDocument.evaluate(this._valuesPath[c],row,this._ns,9,null).singleNodeValue}
							else{return row.ownerDocument.evaluate(this._valuePath,row,this._ns,7,null).snapshotItem(c)}}}
