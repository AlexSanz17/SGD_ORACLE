 ...
 * manager. The service manager can create a few basic services, whose
 * implementations are:
 * <ul>
 ...
 * </ul>
 * 
 * Other services can be inserted into the service manager by using its XSet
 * interface:
 * 
 * <pre>
...
public class Bootstrap{
	 ...
	 */
	static public XComponentContext createInitialComponentContext(Hashtable<String,ComponentContextEntry> context_entries) throws Exception{
		...
		XSingleComponentFactory smgr_fac=(XSingleComponentFactory) UnoRuntime.queryInterface(XSingleComponentFactory.class,xImpLoader.activate("com.sun.star.comp.servicemanager.ServiceManager",null,null,null));

		// Create an instance of the ServiceManager
		XMultiComponentFactory xSMgr=(XMultiComponentFactory) UnoRuntime.queryInterface(XMultiComponentFactory.class,smgr_fac.createInstanceWithContext(null));
		...
		if(context_entries == null)
			context_entries=new Hashtable<String,ComponentContextEntry>(1);
		// add smgr
		...
		// post init smgr
		xInit=(XInitialization) UnoRuntime.queryInterface(XInitialization.class,xSMgr);
	 ...
	 */
	static public final XComponentContext defaultBootstrap_InitialComponentContext() throws Exception{
	 ...
	 * @see cppuhelper/defaultBootstrap_InitialComponentContext()
	 */
	static public final XComponentContext defaultBootstrap_InitialComponentContext(String ini_file,Hashtable<?,?> bootstrap_parameters) throws Exception{
		...
		return (XComponentContext) UnoRuntime.queryInterface(XComponentContext.class,cppuhelper_bootstrap(ini_file,pairs,Bootstrap.class.getClassLoader()));
	}

	static private boolean m_loaded_juh=false;

	static private native Object cppuhelper_bootstrap(String ini_file,String bootstrap_parameters[],ClassLoader loader) throws Exception;
	...
	public static final XComponentContext bootstrap(URLClassLoader loader) throws BootstrapException{
		...
		try{
			...
			// File fOffice =
			// NativeLibraryLoader.getResource(Bootstrap.class.getClassLoader(),
			// sOffice );
			File fOffice=NativeLibraryLoader.getResource(loader,sOffice);
			if(fOffice == null){
				fOffice=new File(SigedProperties.getProperty(SigedProperties.SigedPropertyEnum.OPENOFFICE_RUTA));
			}
			if(fOffice==null)
			...
			cmdArray[2]="-headless";
			cmdArray[3]="-nofirststartwizard";
			cmdArray[4]="-display";
			cmdArray[5]=":99";
			//cmdArray[6]="-accept=pipe,name=" + sPipeName + ";urp;";
			cmdArray[6]="-accept=socket,host=localhost,port=8100;urp;StarOffice.ServiceManager";
